...
- Episode screen-capture thumbnails: `tv_series_files.thumb_path`
  (`''` pending — reset when the file's bytes change, `'unavailable'` = grab
  failed, else the path; a transient gate-acquire failure (`video.ErrBusy`)
  never writes the sentinel). Generated by the `tv_thumb` job (chained after
  `tv_probe`) as one frame grab per episode file — at 25% of the stored
  duration (else 120s), with a fingerprint-detected intro as a **floor**: the
  point is pushed to intro-end+30s only when it would land inside/near the
  intro (never anchored at the intro's end — the fingerprint end undershoots
  the visuals and anchoring put title cards in the thumbs), and a
  fall-toward-start ladder for short files. Files are
  id-keyed `ep_<fileid>.webp` under **`DataDir/thumbs/episodes`** —
  deliberately not `thumbs/tv`, which thumbgc sweeps against
  `tv_series_art`/`people` references. Deleted by the scanner's prune pass and
  by `librariesDelete` (no scan runs after a library delete); extras are
  excluded (`is_extra=0`). A lossless container repair does NOT reset the
  thumb — the remux preserves the video bits, so the grabbed frame is
  identical.
...
### Derived artifacts
- Grid thumbnails: `DataDir/thumbs/photos/photo_<id>.webp` (480px, `photo_thumb` job, missing-only). `thumb_path` states: `''` pending, `unavailable` = generation failed (e.g. grid-HEIC on ffmpeg <7 — grid placeholder, retried when bytes change), else the file path. A transient ffmpeg-gate acquire failure (`video.ErrBusy`) is **not** a generation failure — the row stays `''` and the next `photo_thumb` run retries it.
- Display renditions: `photo_<id>_full.webp` (2048px) for formats a browser can't decode (HEIC/TIFF) — **pre-generated by the `photo_thumb` job** so the viewer's first look is a cache hit, with a lazy on-first-view backfill (`EnsureRendition`, singleflighted per id so concurrent viewers share one encode); browser-displayable formats serve the original. When a file's bytes change in place the scanner deletes the cached rendition (the thumb regenerates via `thumb_path=''`; the rendition has no column to reset).
- Both are id-keyed and deleted by the prune pass when their row goes; a moved file's thumbs regenerate under the surviving row's id rather than transfer. **Deleting a photos library** reaps every row's thumb + rendition files in the same request (`librariesDelete`) — no scan can run afterward to prune them, and thumbgc doesn't cover `thumbs/photos`.
...
### Path & ownership
- One row per file in `books` (`UNIQUE(library_id, abs_path)`), `format` = `epub` | `cbz` | `pdf` (readable) | `mobi`/`azw`/`azw3`/`fb2`/`cbr` (Tier 2: listed, not readable) | `''` (recognized extension but unparseable — corrupt Tier-1). **No matching and no external metadata** — everything on the row is embedded (EPUB OPF title/creator, PDF Info dict) or filename-derived (underscores read as spaces), so *everything* re-derives on rescan except reading progress.
- `page_count` = spine items (EPUB) / image pages (CBZ); 0 = unknown (PDF, Tier 2).
- The reader re-parses the archive on open (spine/page list is **not** stored) — books are small and a zip central-directory read is milliseconds, so there is no stored reading-order blob to go stale.

### Scan logic (`internal/bookscan/scanner.go`)
- Walk skips dot-dirs, dot-files, and `@eaDir`. Unchanged (size+mtime) = pure skip; changed bytes re-parse and reset `thumb_path` to `''` so the chained `book_thumb` job regenerates the cover.
- A corrupt Tier-1 file degrades to `format=''` (listed with a generic cover, no Read button) and **never fails the scan**.
- Scan chain: `book_scan` → `book_thumb`. No probe/match/integrity/loudness — books never touch ffmpeg outside the cover thumb.

### Manual state (NOT re-derivable)
- `book_reading_progress` (PK `book_id`): `spine_index` + `scroll_fraction` + earn-only `completed` (upsert `MAX(completed, excluded.completed)` — the reader reports completion only when a session actually reached the end). Move-relink transfers it on the `(file_size_bytes, mtime_unix)` signature, strictly 1:1, before prune.
- Caveat: `spine_index` is positional — if a *changed* EPUB reorders its spine, the stored index points at a different chapter (accepted; the byte-change also resets the cover, and progress positions are only meaningful against the bytes they were read from).
