...
- Episode screen-capture thumbnails: `tv_series_files.thumb_path`
  ...
  `tv_probe`) as one frame grab per episode file — just past a
  fingerprint-detected intro when one exists, else 25% of the stored duration,
  else 120s, with a fall-toward-start ladder for short files. Files are
