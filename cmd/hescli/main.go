...
Commands:
  ...
  library add --name N --type T --path P   Add a library (type: music|tv|movies|home_media|books|audiobooks)
...
var cmdHelp = map[string]string{
	"library":    "hescli library <list|add|rm>\n  list                            List libraries\n  add --name N --type T --path P  Add a library (type: music|tv|movies|home_media|books|audiobooks)\n  rm <id>                         Delete a library\n",
...
func libraryCmd(c *client, args []string) error {
	...
	case "add":
		...
		typ := fs.String("type", "", "library type (music|tv|movies|home_media|books|audiobooks)")
...
_hescli() {
  ...
  local cfgkeys="tmdb_api_key fanarttv_api_key audiodb_api_key lastfm_api_key opensubtitles_api_key opensubtitles_user_agent integrity_autorepair watch_enabled job_resume_enabled lyrics_enabled update_check_enabled power_button_enabled display_control_enabled display_mode noise_remote_url default_audio_lang default_subtitle_lang subtitles_default_on subtitle_size subtitle_bg subtitle_position media_root"
  local types="music tv movies home_media books audiobooks"
